...
def generate_text(session, input_text, tokenizer, max_length=128, temperature=1.0, top_k=1):
    generated_logits = []  # Store all logits locally
    
    size = 8
    # Tokenize input
    inputs = tokenizer(input_text, return_tensors="np", padding="max_length", truncation=True, max_length=8)
    ...
    for _ in range(max_length):
        ...
        # Ensure output1 has shape [1,1,8,768]
        # if logits.shape != (1, 1, 8, 768):
        #     raise ValueError(f"Unexpected output1 shape: {logits.shape}, expected (1,1,8,768)")
        ...
        generated_logits.append(logits.detach().cpu().numpy())
        
        # Get the next token using top-k sampling
        probs = torch.nn.functional.softmax(logits, dim=-1)
        top_k_probs, top_k_indices = torch.topk(probs, top_k, dim=-1)
        next_token = top_k_indices[torch.multinomial(top_k_probs, 1)]
        # Fix dimension here. .reshape(1, 1, -1)
        input_ids = next_token.numpy()#.reshape(1, 1, 8, 768) #np.concatenate([input_ids, next_token.numpy()], axis=-1)

        # input_ids = np.squeeze(input_ids, axis=0)

        # TODO: Check shape - the new token causes a problem.
        # print(input_ids.shape)
    # print(generated_logits)

    # TODO: Check dimensions and extracted logits and tokens.
    # There is a problem with them in order to extract the output.
    decoded_tokens = tokenizer.batch_decode(generated_logits[0], skip_special_tokens=True, clean_up_tokenization_spaces=True) #[tokenizer.decode(t) for t in input_ids[0]]
