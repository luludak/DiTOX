...
class Executor:
    def __init__(self, models_data, images_data, connection_data, extra_folder):
        ...
        self.error_base_folder = connection_data["error_base_folder"] if "error_base_folder" in connection_data else self.output_model_folder + "/errors"
        self.module = None
        self.device = None
    ...
    def prepare(self, variant="", remote=None):
        ...
        if(host_type == "local_no_rpc" or host_type == "local" or remote == None):
            ...
            model_path = self.input_model_folder + variant
            ...
            try:
                ...
                if("onnx" in self.library):
                    ...
                    if(not self.library.endswith("library")):
                        model_to_execute = torch.load(self.model_path)
                    else:
    ...
    def reset_module(self, variant):
        ...
        self.module = GraphModuleDebug(
            ...
            self.json_str,
            dump_root=self.output_images_base_folder + "/" + self.library + "/debug/")
        # print(self.output_images_base_folder + "/" + self.library + "/debug/")
        print("-------------------------------------------------Module Reset.")
